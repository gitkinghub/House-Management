...
def calculate_house_stats():
    for house in House.objects.all():
        ...
        house.completed_tasks_count = completed_task_count
        house.not_completed_tasks_count = total_tasks - completed_task_count
        house.save()

if not BT.objects.filter(verbose_name='calculate_house_stats').exists():
    calculate_house_stats(repeat=BT.DAILY, verbose_name="Calculate house stats", priority=0)
